...
function TipoDocumento(){  
    ...
    const fetchList = async () => {
        ...
        try {
            const response = await getDocumentTypeListPaginado(`?${q}`);
    ...
    const editAction = (row) => {
        history.push(`/documenttype/edit/${row.original.id}`)
    }
    ...
    const goPageCreate = () => {
        history.push("/documenttype/create")
    }

    const handleDelete = async () => {
        setDeleting(true)   
        try {
            await deleteDocumentType(selectedIdDelete);
            fetchList()
            setDeleting(false)
    ...
    const cardHandleList = (
        ...
        <Row>
            <Col xs="12" xl="12">
                <TableLoader columns={[{name: "Descripción", width: '60%'}, {name: "Habilitado", width: '30%'}, {name: "Acciones", width: '10%'}]} />
            </Col>
        </Row> :
        <Row>
            <Col xl="12">                                    
  ...
  export default withRouter(TipoDocumento)