...
function EditReservation() {
  ...
  return (
    <div className="page-content">
      <Container fluid>
        <Breadcrumbs title={"Reservación"} breadcrumbItem={"Reservación"} />

        <Row className="pb-5">
          <Col lg="12">
            {states.loading && (
              <FormLoader fields={fields} titleCard="Editar Reservación" />
            )}
            {states.success && (
              <CardMain
                title="Editar Reservación"
                children={<FormReserva item={item} btnTextSubmit="Guardar" />}
              />
            )}
            {states.error && (
              <ErrorEntity
