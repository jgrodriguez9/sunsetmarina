...
export default function FormBoatCrew({item, btnTextSubmit="Aceptar"}){
    ...
    const fetchBoatAllApi = async () => {
        try {
            ...
            setBoatOpt(response.map((boat) => ({label: boat.name, value: boat.id})))
        } catch (error) {
            setBoatOpt([])
        }
    }
    ...
    return(
        ...
        >
            ...
            <Row>
                <Col xs="12" md="3">
                    <Label htmlFor="boat" className="mb-0">Bote</Label>
                    <Select
                        value={boatDefault}
                        onChange={(value) => {
                    ...
                    {
                        formik.errors.boat &&
                        <div className="invalid-tooltip d-block">{formik.errors.boat?.id}</div>
                    }
                </Col>
            </Row>
            <hr />

            {
                ...
                <div className="d-flex">
                    ...
                    <Link to="/boatcrew" className="btn btn-danger">Cancelar</Link>
                </div>
            }
