...
export default function FormDocumentClient({item, setOpenModalAdd, setRefetch}){
    ...
    const fetchDoumentTypeAllApi = async () => {
        try {
            ...
            setDocumentTypeOpt(response.filter(e=>e.enabled).map((boat) => ({label: boat.description, value: boat.id})))
        } catch (error) {
            setDocumentTypeOpt([])
        }
    }
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            ...
            Object.entries(getObjectValid(values)).forEach(entry => {
                ...
                if(key === 'reminderDate'){
                    formData[key] = moment(values.reminderDate).format('YYYY-MM-DD')
                }else{
                    formData[key] = value
                }                
            });
            ...
            <Row>
                <Col xs="12" md="5">
                    <Row>
                        <Col xs="12" md="12">
                            <div className="mb-3">
                                <Label htmlFor="boatType" className="mb-0">Tipo de documento</Label>
                                <Select
                                    value={formik.values.documentType ? 
                                        {
                                            value: formik.values.documentType, 
                                            label: documentTypeOpt.find(it=>it.value === formik.values.documentType).label ?? ''} :
                                             null}
                                    onChange={(value) => {
                                        formik.setFieldValue('documentType', value?.value ?? '')
                                    }}
                        ...
                        <Col xs="12" md="12">
                            <div className="mb-3">
                                <Label htmlFor="file" className="mb-0">Archivo</Label>
                                <Input 
                                ...
                                />
                                {
                                    ...
                                    <div className="invalid-tooltip d-block">{formik.errors.fileToUpload}</div>
                                }
                            </div>
                        </Col>
                        <Col xs="12" md="12">
                            ...
                            />
                            <span className="text-muted">Escoger una fecha si desea una notificación de recordatorio para ese día</span>
                        </Col>
                    </Row>
                </Col>
                <Col xs="12" md="7">
