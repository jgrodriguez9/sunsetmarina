...
export default function PaginateTable({columns, data, initialState}){
    ...
    const getPageIndexOptions = (maxNumberOfOptions, pageSize, pageI) => {
        ...
        if (lastPageIndex <= maxNumberOfOptions) {
          ...
          for (var i = pageI - (pivot - 1); options.length < maxNumberOfOptions; i++) {
            options.push(i + 1);
          }
        }    
        return options;
                        ...
                        <div className="d-flex align-items-center">
                            <div className="me-auto">
                                <span className="text-muted fs-08">
                                    ...
                                    >
                                    {
                                        rows.length < 10 ? <option value={rows.length}>{rows.length}</option> :
                                        limites.map((item) => (
                                        <option value={item} key={item}>{item}</option>
                                        ))

                                    }
                                    </select> de {rows.length}
                                </span>
                            </div>
                            <div>
                                <Pagination className="pagination pagination-rounded justify-content-end paginate-margin-ul-none">
                                    ...
                                    {
                                    getPageIndexOptions(5, pageOptions.length, pageCount).map((item, i) => (
                                        <PaginationItem active={item === pageIndex + 1} key={i}>
                                            <PaginationLink
                                            onClick={() => {
                                                console.log(item)
                                                gotoPage(item-1)
